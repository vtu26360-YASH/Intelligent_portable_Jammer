...
# Author: Vel Tech
# Description: Advanced Narrowband Spot Jammer
...
class veltech_advanced_jammer(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "veltech_advanced_jammer")
        ...
        # Variables
        ##################################################
        self.target_freq = target_freq = 0
        ...
        self._target_freq_range = qtgui.Range(-500000, 500000, 10000, 0, 200)
        self._target_freq_win = qtgui.RangeWidget(self._target_freq_range, self.set_target_freq, "Enemy Signal Offset (Hz)", "counter_slider", float, QtCore.Qt.Horizontal)
        self.top_grid_layout.addWidget(self._target_freq_win, 3, 4, 1, 1)
        for r in range(3, 4):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(4, 5):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._jammer_amp_range = qtgui.Range(0, 1, 0.1, 0, 200)
        self._jammer_amp_win = qtgui.RangeWidget(self._jammer_amp_range, self.set_jammer_amp, "Jammer Power (AI Control)", "counter_slider", float, QtCore.Qt.Horizontal)
        self.top_grid_layout.addWidget(self._jammer_amp_win, 3, 5, 1, 1)
        for r in range(3, 4):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(5, 6):
            self.top_grid_layout.setColumnStretch(c, 1)
        self.throttle = blocks.throttle(gr.sizeof_gr_complex*1, samp_rate,True)
        self.target_signal = analog.sig_source_c(samp_rate, analog.GR_COS_WAVE, target_freq, 0.5, 0, 0)
        ...
        self.jammer_lpf = filter.fir_filter_ccf(
            ...
            firdes.low_pass(
                ...
                window.WIN_HAMMING,
                6.76))
        self.jammer_lo = analog.sig_source_c(samp_rate, analog.GR_COS_WAVE, target_freq, 1, 0, 0)
    ...
    def set_target_freq(self, target_freq):
        ...
        self.target_signal.set_frequency(self.target_freq)
        self.jammer_lo.set_frequency(self.target_freq)
...
def main(top_block_cls=veltech_advanced_jammer, options=None):
